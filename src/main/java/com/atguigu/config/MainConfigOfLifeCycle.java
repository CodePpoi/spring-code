...
//bean的创建(构造函数，比如Car里面的无参构造函数) 初始化(对象创建完成，并赋值好(AbstractAutowireCapableBeanFactory.populateBean)，调用初始化方法) 销毁(容器关闭的时候)
/**
 * 1) 指定初始化和销毁方法:
 *  通过@Bean指定init-method和destroy-method
 * 2) 通过让Bean实现InitializingBean.afterPropertiesSet(定义初始化逻辑),
 * DisposableBean.destroy(定义销毁逻辑)接口
 ...
 *
 * 总结一下, 就是:@Autowired 生命周期注解比如@PostConstruct @Async 等等功能，都是通过BeanPostProcessor实现的
 *
 * */
